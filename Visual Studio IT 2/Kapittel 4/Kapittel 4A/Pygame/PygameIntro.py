...
hinder = Hinder(150, 250, 20, (0, 0, 255), vindu, 0.08+0.001, 0.12+0.001)
...
while fortsett:
    ...
    if (spiller.x - spiller.radius) <= 0:
        spiller.x = spiller.radius  # Prevent moving beyond the left boundary
    elif (spiller.x + spiller.radius) >= spiller.vindusobjekt.get_width():
        spiller.x = spiller.vindusobjekt.get_width() - spiller.radius  # Prevent moving beyond the right boundary

    if (spiller.y - spiller.radius) <= 0:
        spiller.y = spiller.radius  # Prevent moving beyond the left boundary
    elif (spiller.y + spiller.radius) >= spiller.vindusobjekt.get_height():
        spiller.y = spiller.vindusobjekt.get_height() - spiller.radius  # Prevent moving beyond the right boundary
